...
def upload(request):
    if request.method == "POST":
        form = ImagePostForm(request.POST, request.FILES)  # Create the form instance with POST data and files
        if form.is_valid():
            title = form.cleaned_data['title']
            slug = slugify(title)
            user = request.user
            author = form.cleaned_data['user']
            excerpt = form.cleaned_data['text']
        ...
        form = ImagePostForm()  # Create an empty form instance
